 ...
 *  for libcs220 and libbeaengine Specifically, the course include
 *  directory must be included in the compiler include-path using the
 *  -I option which is usually defined using make's CPPFLAGS.
 *
 *  Link Time: has a depency on libdl, libbeaengine and libcs220,
 *  hence -ldl, -lbeaengine and -lcs220 must be specified in make's
